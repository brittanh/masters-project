"""
================================================================================================
                          Test for iNMPC module using nl files
...
m.za_e = Disturbance(m.t, rule=_za_e, forecast='PERFECT')
def _za_w(c_time):
  return 20 + 5.0*sin(c_time*3.14159/60)
m.za_w = Disturbance(m.t, rule=_za_w, forecast='PERFECT')  
def _za_g(c_time):
  return 0
m.za_g = Disturbance(m.t, rule=_za_g, forecast='PERFECT')  
...
def _PMB(m,l,i,j):
  ...
  if i == 1: # Condenser
    ...
    return m.dxdt[l,i,j] == ( (m.FA[i]*(za[j][l]-m.x[l,i,j])) + (m.FG[i]*(m.zg[j]-m.x[l,i,j])) + (m.L[l,i-1]*(m.x[l,i-1,j]-m.x[l,i,j])) + (m.V[l,i+1]*(m.y[l,i+1,j]-m.x[l,i,j])) - (m.V[l,i]*(m.y[l,i,j]-m.x[l,i,j])) )/m.ML[l,i]
...
def _obj(m):
  ...
  return sum( 0.5*(f[i]+f[t_tmp[t_tmp.index(i)-1]])*(i - t_tmp[t_tmp.index(i)-1]) for i in t_tmp if i>m.to)  
m.ObjectiveFunction = Objective(rule=_obj, sense=minimize)

## ================================================================================================
## iNMPC solution
## ================================================================================================
CONTROLLER = IdealNMPC(model=m, model_time=m.t, prediction_horizon=10,
...
CONTROLLER.solve('ipopt', options=[('linear_solver', 'ma57'), 
    ...
    file_name='EDC_iNMPC_nl_CF_test', dynamic_plot=True)
